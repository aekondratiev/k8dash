...
>In this branch, we have added components to display a section called `Tasks` which will include `Jobs`, `CronJobs`, `knerrirs`, and `scheduledknerrir`.
>
>There are also subsections for all the resources mentioned above. In these pages you will find detailed info on the selected resource.
>
>Most of the logic is implemented using already exisiting code. New pages have been added and we ate registering the knerrir api.
>
>If you create a PR for master, you can easily see all the changes.
>
>This extension of the base code is not ideal and the whole thing should be revisted if is being seriously considered. This was just an trial run. Even the sorting logic does not work for everyting.
>
>The deployment logic remains the same.
